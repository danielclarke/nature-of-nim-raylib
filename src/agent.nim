...
proc newFlowField(width, height: int; t: float): FlowField =
  ...
  for i in 0 ..< width:
    var f: seq[Vec2]
    for j in 0 ..< height:
      # echo noise(i.float + 0.137, j.float + 0.653, 777.777)
      f.add(10.0 * vec2FromAngle(2 * PI * noise(i.float / 25.0, j.float / 25.0, t)))
      # f.add(randVec2(Vec2(x: -1.0, y: -1.0), Vec2(x: 1.0, y: 1.0)).norm * 10.0)
    field.add(f)
